...
item *makeItemInto(item *theItem, unsigned long itemCategory, short itemKind) {
  ...
  case WEAPON:
    ...
    if (rand_percent(40)) {
      ...
      } else if (rand_range(3, 10) * ((theItem->flags & ITEM_ATTACKS_STAGGER) ? 2 : 1)
  ...
  case GOLD:
    ...
    theItem->quantity = rand_range(50 + rogue.depthLevel * 10 * gameConst->depthAccelerator,
                                   100 + rogue.depthLevel * 15 * gameConst->depthAccelerator);
    break;
  case AMULET:
    theEntry = NULL;
...
item *placeItem(item *theItem, short x, short y) {
  ...
  removeItemFromChain(
      theItem, floorItems); // just in case; double-placing an item will result in game-crashing loops in the item list
...
void fillItemSpawnHeatMap(unsigned short heatMap[DCOLS][DROWS], unsigned short heatLevel, short x, short y) {
  ...
  for (dir = 0; dir < 4; dir++) {
    ...
    if (coordinatesAreInMap(newX, newY)
        ...
        && isPassableOrSecretDoor(newX, newY) && heatLevel < heatMap[newX][newY]) {
...
void populateItems(short upstairsX, short upstairsY) {
  ...
  const int POW_GOLD[] = {// b^3.05, with b from 0 to 25:
                          0,    1,    8,    28,   68,   135,  236,  378,  568,   813,   1122,  1500,  1956,
                          2497, 3131, 3864, 4705, 5660, 6738, 7946, 9292, 10783, 12427, 14232, 16204, 18353};
...
#define aggregateGoldUpperBound(d) (POW_GOLD[d] + 420 * (d))
  const fixpt POW_FOOD[]
      = {// b^1.35 fixed point, with b from 1 to 50 (for future-proofing):
         65536,   167059,   288797,   425854,   575558,   736180,   906488,   1085553,  1272645,  1467168,
         1668630, 1876612,  2090756,  2310749,  2536314,  2767208,  3003211,  3244126,  3489773,  3739989,
         3994624, 4253540,  4516609,  4783712,  5054741,  5329591,  5608167,  5890379,  6176141,  6465373,
         6758000, 7053950,  7353155,  7655551,  7961076,  8269672,  8581283,  8895856,  9213341,  9533687,
         9856849, 10182782, 10511443, 10842789, 11176783, 11513384, 11852556, 12194264, 12538472, 12885148};
  ...
  for (i = 0; i < numberOfItems; i++) {
    ...
    if ((rogue.foodSpawned + foodTable[RATION].power / 3) * 4 * FP_FACTOR
      ...
      for (j = 0; j < gameConst->numberMeteredItems; j++) {
        ...
        if (meteredItemsGenerationTable[j].levelScaling != 0
            && rogue.meteredItems[j].numberSpawned * meteredItemsGenerationTable[j].genMultiplier
                       + meteredItemsGenerationTable[j].genIncrement
    ...
    for (j = 0; j < gameConst->numberMeteredItems; j++) {
      if ((theItem->category & meteredItemsGenerationTable[j].category)
          && theItem->kind == meteredItemsGenerationTable[j].kind) {
        if (j >= gameConst->numberScrollKinds) {
          if (D_MESSAGE_ITEM_GENERATION)
            printf("\n(!)  Depth %i: generated an %s potion at %i frequency", rogue.depthLevel,
                   potionTable[j - gameConst->numberScrollKinds].name, rogue.meteredItems[j].frequency);
        } else {
          if (D_MESSAGE_ITEM_GENERATION)
            printf("\n(?)  Depth %i: generated an %s scroll at %i frequency", rogue.depthLevel, scrollTable[j].name,
                   rogue.meteredItems[j].frequency);
        }
...
boolean itemWillStackWithPack(item *theItem) {
  ...
  if (theItem->category & GEM) {
    ...
    for (tempItem = packItems->nextItem; tempItem != NULL && tempItem->quiverNumber != theItem->quiverNumber;
         tempItem = tempItem->nextItem)
      ;
    return (tempItem ? true : false);
  }
...
void pickUpItemAt(short x, short y) {
  ...
  if ((theItem->flags & ITEM_KIND_AUTO_ID) && tableForItemCategory(theItem->category)
...
void conflateItemCharacteristics(item *newItem, item *oldItem) {
  ...
  newItem->flags |= (oldItem->flags
                     & (ITEM_MAGIC_DETECTED | ITEM_IDENTIFIED | ITEM_PROTECTED | ITEM_RUNIC | ITEM_RUNIC_HINTED
                        | ITEM_CAN_BE_IDENTIFIED | ITEM_MAX_CHARGES_KNOWN));
...
void checkForDisenchantment(item *theItem) {
  ...
  if ((theItem->flags & ITEM_RUNIC)
      && (((theItem->category & WEAPON) && theItem->enchant2 < NUMBER_GOOD_WEAPON_ENCHANT_KINDS)
          || ((theItem->category & ARMOR) && theItem->enchant2 < NUMBER_GOOD_ARMOR_ENCHANT_KINDS))
      && theItem->enchant1 <= 0) {
  ...
  if ((theItem->category & STAFF) && newEnchant < 2 || (theItem->category & CHARM) && newEnchant < 1
    ...
    if (enchantmentKnown) {
      ...
      if (theItem->category & WEAPON) {
        ...
        theItem->charges
            = gameConst->armorDelayToAutoID; // this many turns until it reveals its enchants and whether runic
...
boolean swapItemEnchants(const short machineNumber) {
  ...
  for (i = 0; i < DCOLS; i++) {
    for (j = 0; j < DROWS; j++) {
      ...
      if (tempItem && pmap[i][j].machineNumber == machineNumber && cellHasTMFlag(i, j, TM_SWAP_ENCHANTS_ACTIVATION)
...
void updateFloorItems() {
  ...
  for (theItem = floorItems->nextItem; theItem != NULL; theItem = nextItem) {
    ...
    if (cellHasTerrainFlag(x, y, T_MOVES_ITEMS)) {
      pos loc;
      getQualifyingLocNear(&loc, x, y, true, 0, (T_OBSTRUCTS_ITEMS | T_OBSTRUCTS_PASSABILITY), (HAS_ITEM), false,
                           false);
      removeItemFrom(x, y);
    ...
    if (pmap[x][y].machineNumber && pmap[x][y].machineNumber == pmapAt(player.loc)->machineNumber
    ...
    if (cellHasTMFlag(x, y, TM_SWAP_ENCHANTS_ACTIVATION) && pmap[x][y].machineNumber) {
      ...
      while (nextItem != NULL && pmap[x][y].machineNumber == pmapAt(nextItem->loc)->machineNumber
...
void call(item *theItem) {
  ...
  if (tableForItemCategory(theItem->category) && !(tableForItemCategory(theItem->category)[theItem->kind].identified)) {
...
//  yellow. BaseColor itself will be the color that the name reverts to outside of these colored portions.
void itemName(item *theItem, char *root, boolean includeDetails, boolean includeArticle, const color *baseColor) {
  char buf[DCOLS * 5], pluralization[10], article[10] = "", runicName[30], grayEscapeSequence[5],
                                          purpleEscapeSequence[5], yellowEscapeSequence[5], baseEscapeSequence[5];
  color tempColor;
  ...
  case SCROLL:
    if (scrollTable[theItem->kind].identified || rogue.playbackOmniscience) {
      ...
      sprintf(root, "scroll%s called %s%s%s", pluralization, purpleEscapeSequence, scrollTable[theItem->kind].callTitle,
              baseEscapeSequence);
    } else {
  ...
  case POTION:
    if (potionTable[theItem->kind].identified || rogue.playbackOmniscience) {
      ...
      sprintf(root, "potion%s called %s%s%s", pluralization, purpleEscapeSequence, potionTable[theItem->kind].callTitle,
              baseEscapeSequence);
    } else {
...
static int enchantMagnitude() { return tableForItemCategory(SCROLL)[SCROLL_ENCHANTING].power; }
...
boolean isVowelish(char *theChar) {
  ...
  if (stringsMatch(str, "UNI") // Words that start with "uni" aren't treated like vowels; e.g., "a" unicorn.
      || stringsMatch(str,
                      "EU")) { // Words that start with "eu" aren't treated like vowels; e.g., "a" eucalpytus staff.
...
void itemDetails(char *buf, item *theItem) {
  ...
  if (tableForItemCategory(theItem->category)
    ...
    case SCROLL:
      sprintf(buf2,
              "%s parchment%s %s covered with indecipherable writing, and bear%s a title of \"%s.\" Who knows what %s "
              "will do when read aloud?",
    ...
    case RING:
      sprintf(buf2,
              "This metal band is adorned with a%s %s gem that glitters in the darkness. Who knows what effect it has "
              "when worn? ",
    ...
    case AMULET:
      strcpy(
          buf2,
          "Legends are told about this mysterious golden amulet, and legions of adventurers have perished in its "
          "pursuit. Unfathomable riches await anyone with the skill and ambition to carry it into the light of day.");
      break;
    case GEM:
      sprintf(buf2,
              "Faint golden lights swirl and fluoresce beneath the stone%s surface. Lumenstones are said to contain "
  ...
  if (carried && theItem->originDepth > 0) {
    sprintf(buf2, " (You found %s%s on depth %i.) ", singular ? "it" : "them",
            // items in an item-choice vault are "keys", but are not a KEY
            // we ignore any actual KEY, as those might not be on an altar
            theItem->flags & ITEM_IS_KEY && !(theItem->category & KEY) ? " in a vault" : "", theItem->originDepth);
    strcat(buf, buf2);
  }
    ...
    if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
      ...
      if (strengthModifier(theItem)) {
        sprintf(
            ...
            strengthModifier(theItem) / (double)FP_FACTOR, whiteColorEscape,
      ...
      if (theItem->category & WEAPON) {
        ...
        sprintf(buf2, "It will reveal its secrets if worn for %i%s turn%s. ", theItem->charges,
                (theItem->charges == gameConst->armorDelayToAutoID ? "" : " more"), (theItem->charges == 1 ? "" : "s"));
      }
      strcat(buf, buf2);
    }
      ...
      if (theItem->category & WEAPON) {
        ...
        sprintf(buf2,
                ...
                ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience)
                    ? ""
                    : ", assuming it has no hidden properties,",
    ...
    if (theItem->category & WEAPON) {
          ...
          if (theItem->enchant2 == W_SLAYING) {
            ...
            if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
              sprintf(buf2,
                      ...
                      "and attack power equal to your own, and will dissipate %i turns later. (If the %s is enchanted, "
                      "%i image%s will appear %i%% of the time, and will last %i turns.)",
              ...
              sprintf(buf2,
                      "Sometimes, when it hits an enemy, spectral %ss will spring into being with accuracy and attack "
                      "power equal to your own, and will dissipate shortly thereafter.",
                      theName);
            }
            strcat(buf, buf2);
          } else {
            ...
            if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
              ...
              case W_PARALYSIS:
                ...
                strcat(buf, buf2);
                nextLevelState
                    = (int)(weaponParalysisDuration(enchant + enchantMagnitude() * enchantIncrement(theItem)));
                break;
              case W_SLOWING:
      ...
      if (theItem->flags & ITEM_RUNIC) {
        if ((theItem->flags & ITEM_RUNIC_IDENTIFIED) || rogue.playbackOmniscience) {
          ...
          case A_MULTIPLICITY:
            sprintf(buf2,
                    "When worn, 33%% of the time that an enemy's attack connects, %i allied spectral duplicate%s of "
                    "your attacker will appear for 3 turns. ",
          ...
          case A_ABSORPTION:
            if (theItem->flags & ITEM_IDENTIFIED) {
              sprintf(buf2,
                      "It will reduce the damage of inbound attacks by a random amount between 1 and %i, which is %i%% "
                      ...
                      theName,
                      (armorAbsorptionMax(enchant) == armorAbsorptionMax(enchant + enchantIncrement(theItem))
                           ? "remain at"
                           : "increase to"),
          ...
          case A_REPRISAL:
            if (theItem->flags & ITEM_IDENTIFIED) {
              sprintf(buf2,
                      "Any enemy that attacks you will itself be wounded by %i%% of the damage that it inflicts. (If "
                      "the %s is enchanted, this percentage will increase to %i%%.) ",
            ...
            if (theItem->flags & ITEM_IDENTIFIED) {
              if (theItem->enchant1 > 0) {
                ...
                sprintf(buf2,
                        ...
                        "source %i%% of the time. (If the armor is enchanted, these will increase to %i%% and %i%%.) ",
                ...
                short reflectChance2 = reflectionChance(enchant + enchantMagnitude() * enchantIncrement(theItem));
                sprintf(buf2,
                        "When worn, %i%% of your own spells will deflect from their target -- including directly back "
                        "at you %i%% of the time. (If the armor is enchanted, these will decrease to %i%% and %i%%.) ",
                        reflectChance, reflectChance * reflectChance / 100, reflectChance2,
                        reflectChance2 * reflectChance2 / 100);
              }
            } else {
              strcpy(
                  buf2,
                  "When worn, you will deflect some percentage of incoming spells, determined by enchantment level. ");
            }
            break;
          case A_RESPIRATION:
            strcpy(buf2, "When worn, it will maintain a pocket of fresh air around you, rendering you immune to the "
                         "effects of steam and all toxic gases. ");
            break;
          case A_DAMPENING:
  ...
  case STAFF:
    ...
    if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
      ...
      sprintf(buf2, "\n\nThe %s has a maximum of %i charges, and%s recovers a charge in approximately %lli turns. ",
              theName, theItem->enchant1, new == 0 ? "" : ", with your current rings,",
              FP_DIV(staffChargeDuration(theItem), 10 * ringWisdomMultiplier(new *FP_FACTOR)));
      strcat(buf, buf2);
    }

    if (theItem->lastUsed[0] > 0 && theItem->lastUsed[1] > 0 && theItem->lastUsed[2] > 0) {
      sprintf(buf2, "You last used it %li, %li and %li turns ago. ", rogue.absoluteTurnNumber - theItem->lastUsed[0],
              rogue.absoluteTurnNumber - theItem->lastUsed[1], rogue.absoluteTurnNumber - theItem->lastUsed[2]);
      strcat(buf, buf2);
    } else if (theItem->lastUsed[0] > 0 && theItem->lastUsed[1] > 0) {
      sprintf(buf2, "You last used it %li and %li turns ago. ", rogue.absoluteTurnNumber - theItem->lastUsed[0],
    ...
    if (((theItem->flags & (ITEM_IDENTIFIED | ITEM_MAX_CHARGES_KNOWN)) && staffTable[theItem->kind].identified)
      ...
      case STAFF_LIGHTNING:
        sprintf(buf2,
                "This staff deals damage to every creature in its line of fire; nothing is immune. (If the staff is "
                "enchanted, its average damage will increase by %i%%.)",
                (int)(100
                          * (staffDamageLow(enchant + enchantMagnitude() * FP_FACTOR)
                             + staffDamageHigh(enchant + enchantMagnitude() * FP_FACTOR))
                          / (staffDamageLow(enchant) + staffDamageHigh(enchant))
                      - 100));
        break;
      case STAFF_FIRE:
        sprintf(buf2,
                "This staff deals damage to any creature that it hits, unless the creature is immune to fire. (If the "
                "staff is enchanted, its average damage will increase by %i%%.) It also sets creatures and flammable "
                "terrain on fire.",
                (int)(100
                          * (staffDamageLow(enchant + enchantMagnitude() * FP_FACTOR)
                             + staffDamageHigh(enchant + enchantMagnitude() * FP_FACTOR))
      ...
      case STAFF_BLINKING:
        sprintf(buf2,
                "This staff enables you to teleport up to %i spaces. (If the staff is enchanted, this will increase to "
                "%i spaces.)",
                staffBlinkDistance(enchant), staffBlinkDistance(enchant + enchantMagnitude() * FP_FACTOR));
        break;
      case STAFF_ENTRANCEMENT:
        sprintf(buf2,
                "This staff will compel its target to mirror your movements for %i turns. (If the staff is enchanted, "
      ...
      case STAFF_HASTE:
        sprintf(buf2,
                "This staff will cause its target to move twice as fast for %i turns. (If the staff is enchanted, this "
                "will increase to %i turns.)",
      ...
      case STAFF_DISCORD:
        sprintf(
            buf2,
            "This staff will cause discord for %i turns. (If the staff is enchanted, this will increase to %i turns.)",
            staffDiscordDuration(enchant), staffDiscordDuration(enchant + enchantMagnitude() * FP_FACTOR));
        break;
      case STAFF_CONJURATION:
        sprintf(buf2,
                "%i phantom blades will be called into service. (If the staff is enchanted, this will increase to %i "
                "blades.)",
    ...
    if (((theItem->flags & ITEM_IDENTIFIED) && ringTable[theItem->kind].identified) || rogue.playbackOmniscience) {
      if (theItem->enchant1) {
        ...
        case RING_CLAIRVOYANCE:
          if (theItem->enchant1 > 0) {
            sprintf(buf2,
                    "\n\nThis ring provides magical sight with a radius of %i. (If the ring is enchanted, this will "
                    "increase to %i.)",
        ...
        case RING_REGENERATION:
          sprintf(buf2,
                  "\n\nWith this ring equipped, you will regenerate all of your health in %li turns (instead of %li). "
                  "(If the ring is enchanted, this will decrease to %li turns.)",
                  (long)(turnsForFullRegenInThousandths(enchant) / 1000), (long)TURNS_FOR_FULL_REGEN,
                  (long)(turnsForFullRegenInThousandths(enchant + enchantMagnitude() * FP_FACTOR) / 1000));
          strcat(buf, buf2);
          break;
        case RING_TRANSFERENCE:
          sprintf(buf2,
                  "\n\nDealing direct damage to a creature (whether in melee or otherwise) will %s you by %i%% of the "
                  "damage dealt. (If the ring is enchanted, this will %s to %i%%.)",
                  (theItem->enchant1 >= 0 ? "heal" : "harm"), abs(theItem->enchant1) * 5,
                  (theItem->enchant1 >= 0 ? "increase" : "decrease"), abs(theItem->enchant1 + enchantMagnitude()) * 5);
          strcat(buf, buf2);
          break;
        case RING_WISDOM:
          sprintf(buf2,
                  "\n\nWhen worn, your staffs will recharge at %i%% of their normal rate. (If the ring is enchanted, "
                  "the rate will increase to %i%% of the normal rate.)",
        ...
        case RING_REAPING:
          sprintf(buf2,
                  "\n\nEach blow that you land with a weapon will %s your staffs and charms by 0-%i turns per point of "
                  ...
                  (theItem->enchant1 >= 0 ? "increase" : "decrease"), abs(theItem->enchant1 + enchantMagnitude()));
          strcat(buf, buf2);
          break;
        default:
          break;
      ...
      sprintf(buf2, "\n\nIt will reveal its secrets if worn for %i%s turn%s", theItem->charges,
              (theItem->charges == gameConst->ringDelayToAutoID ? "" : " more"), (theItem->charges == 1 ? "" : "s"));
      strcat(buf, buf2);

      if (!(theItem->flags & ITEM_IDENTIFIED)
          && (theItem->charges < gameConst->ringDelayToAutoID || (theItem->flags & ITEM_MAGIC_DETECTED))) {
  ...
  case CHARM:
    ...
    case CHARM_HEALTH:
      sprintf(buf2,
              ...
              charmHealing(enchant), charmRechargeDelay(theItem->kind, theItem->enchant1),
    ...
    case CHARM_PROTECTION:
      sprintf(buf2,
              "\n\nWhen used, the charm will shield you for up to 20 turns for up to %i%% of your total health and "
              "recharge in %i turns. (If the charm is enchanted, it will shield up to %i%% of your total health and "
              "recharge in %i turns.)",
    ...
    case CHARM_FIRE_IMMUNITY:
      sprintf(buf2,
              "\n\nWhen used, the charm will grant you immunity to fire for %i turns and recharge in %i turns. (If the "
              "charm is enchanted, the immunity will last %i turns and it will recharge in %i turns.)",
              charmEffectDuration(theItem->kind, theItem->enchant1),
              charmRechargeDelay(theItem->kind, theItem->enchant1),
              charmEffectDuration(theItem->kind, theItem->enchant1 + enchantMagnitude()),
              charmRechargeDelay(theItem->kind, theItem->enchant1 + enchantMagnitude()));
    ...
    case CHARM_TELEPATHY:
      sprintf(buf2,
              "\n\nWhen used, the charm will grant you telepathy for %i turns and recharge in %i turns. (If the charm "
              "is enchanted, the telepathy will last %i turns and it will recharge in %i turns.)",
              charmEffectDuration(theItem->kind, theItem->enchant1),
              charmRechargeDelay(theItem->kind, theItem->enchant1),
              charmEffectDuration(theItem->kind, theItem->enchant1 + enchantMagnitude()),
              charmRechargeDelay(theItem->kind, theItem->enchant1 + enchantMagnitude()));
    ...
    case CHARM_LEVITATION:
      sprintf(buf2,
              "\n\nWhen used, the charm will lift you off the ground for %i turns and recharge in %i turns. (If the "
              "charm is enchanted, the levitation will last %i turns and it will recharge in %i turns.)",
              charmEffectDuration(theItem->kind, theItem->enchant1),
              charmRechargeDelay(theItem->kind, theItem->enchant1),
              charmEffectDuration(theItem->kind, theItem->enchant1 + enchantMagnitude()),
              charmRechargeDelay(theItem->kind, theItem->enchant1 + enchantMagnitude()));
    ...
    case CHARM_SHATTERING:
      sprintf(buf2,
              "\n\nWhen used, the charm will dissolve the nearby walls up to %i spaces away, and recharge in %i turns. "
              "(If the charm is enchanted, it will reach up to %i spaces and recharge in %i turns.)",
              charmShattering(enchant), charmRechargeDelay(theItem->kind, theItem->enchant1),
    ...
    case CHARM_GUARDIAN:
      sprintf(buf2,
              ...
              charmGuardianLifespan(enchant), charmRechargeDelay(theItem->kind, theItem->enchant1),
              charmGuardianLifespan(enchant + enchantMagnitude() * FP_FACTOR),
              charmRechargeDelay(theItem->kind, theItem->enchant1 + enchantMagnitude()));
      break;
    case CHARM_TELEPORTATION:
      sprintf(buf2,
              "\n\nWhen used, the charm will teleport you elsewhere in the dungeon and recharge in %i turns. (If the "
              "charm is enchanted, it will recharge in %i turns.)",
              charmRechargeDelay(theItem->kind, theItem->enchant1),
              charmRechargeDelay(theItem->kind, theItem->enchant1 + enchantMagnitude()));
      break;
    case CHARM_RECHARGING:
      sprintf(buf2,
              "\n\nWhen used, the charm will recharge your staffs (though not your wands or charms), after which it "
              "will recharge in %i turns. (If the charm is enchanted, it will recharge in %i turns.)",
              charmRechargeDelay(theItem->kind, theItem->enchant1),
              charmRechargeDelay(theItem->kind, theItem->enchant1 + enchantMagnitude()));
      break;
    case CHARM_NEGATION:
      sprintf(buf2,
              "\n\nWhen used, the charm will emit a wave of anti-magic up to %i spaces away, negating all magical "
              "effects on you and on creatures and dropped items in your field of view. It will recharge in %i turns. "
              "(If the charm is enchanted, it will reach up to %i spaces and recharge in %i turns.)",
              charmNegationRadius(enchant), charmRechargeDelay(theItem->kind, theItem->enchant1),
...
char displayInventory(unsigned short categoryMask, unsigned long requiredFlags, unsigned long forbiddenFlags,
                      boolean waitForAcknowledge, boolean includeButtons) {
  item *theItem;
...
short numberOfMatchingPackItems(unsigned short categoryMask, unsigned long requiredFlags, unsigned long forbiddenFlags,
...
item *keyOnTileAt(short x, short y) {
  ...
  if ((pmap[x][y].flags & HAS_PLAYER) && player.loc.x == x && player.loc.y == y && keyInPackFor(x, y)) {
...
short getLineCoordinates(pos listOfCoordinates[], const pos originLoc, const pos targetLoc, const bolt *theBolt) {
  ...
  const fixpt offsets[][2]
      = {{50, 50}, // center of the square first (coordinates are in %)
         {40, 40}, {60, 40}, {60, 60}, {40, 60}, {50, 30}, {70, 50}, {50, 70}, {30, 50}, {50, 20}, {80, 50},
         {50, 80}, {20, 50}, {50, 10}, {90, 50}, {50, 90}, {10, 50}, {50, 1},  {99, 50}, {50, 99}, {1, 50}};
  ...
  for (offset = 0; offset < numOffsets + 1; offset++) {
    ...
    // vector to target
    step[0]
        = targetLoc.x * FP_FACTOR + offsets[offset < numOffsets ? offset : bestOffset][0] * FP_FACTOR / 100 - point[0];
    step[1]
        = targetLoc.y * FP_FACTOR + offsets[offset < numOffsets ? offset : bestOffset][1] * FP_FACTOR / 100 - point[1];
    ...
    while (true) {
      point[0] += step[0];
      point[1] += step[1];
      listOfCoordinates[listLength]
          = (pos){.x = (point[0] < 0 ? -1 : point[0] / FP_FACTOR), .y = (point[1] < 0 ? -1 : point[1] / FP_FACTOR)};
    ...
    for (int i = 0; i < listLength; i++) {
      ...
      boolean isMonster = monst && !(monst->bookkeepingFlags & MB_SUBMERGED) && !monsterIsHidden(monst, caster);
      ...
      if (x == targetLoc.x && y == targetLoc.y) {
        ...
        if ((!targetsEnemies && !targetsAllies) || (targetsEnemies && isMonster && isEnemyOfCaster)
        ...
        break; // we don't care about anything beyond the target--if the player did, they would have selected a farther
               // target
      }
...
void getImpactLoc(pos *returnLoc, const pos originLoc, const pos targetLoc, const short maxDistance,
                  const boolean returnLastEmptySpace, const bolt *theBolt) {
  pos coords[DCOLS + 1];
  short i, n;
  creature *monst;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (monst && !monsterIsHidden(monst, monsterAtLoc(originLoc)) && !(monst->bookkeepingFlags & MB_SUBMERGED)) {
...
boolean polymorph(creature *monst) {
  ...
  } while (monsterCatalog[newMonsterIndex].flags
               & (MONST_INANIMATE
                  | MONST_NO_POLYMORPH) // Can't turn something into an inanimate object or lich/phoenix/warden.
...
void rechargeItems(unsigned long categories) {
  ...
  for (tempItem = packItems->nextItem; tempItem != NULL; tempItem = tempItem->nextItem) {
    if (tempItem->category & categories & STAFF) {
      ...
      tempItem->enchant2 = (tempItem->kind == STAFF_BLINKING || tempItem->kind == STAFF_OBSTRUCTION ? 10000 : 5000)
                           / tempItem->enchant1;
    }
...
void negationBlast(const char *emitterName, const short distance) {
  ...
  for (creatureIterator it = iterateCreatures(monsters); hasNextCreature(it);) {
    creature *monst = nextCreature(&it);
    if ((pmapAt(monst->loc)->flags & IN_FIELD_OF_VIEW)
        && (player.loc.x - monst->loc.x) * (player.loc.x - monst->loc.x)
                   + (player.loc.y - monst->loc.y) * (player.loc.y - monst->loc.y)
               <= distance * distance) {
  ...
  for (theItem = floorItems; theItem != NULL; theItem = theItem->nextItem) {
    if ((pmapAt(theItem->loc)->flags & IN_FIELD_OF_VIEW)
        && (player.loc.x - theItem->loc.x) * (player.loc.x - theItem->loc.x)
                   + (player.loc.y - theItem->loc.y) * (player.loc.y - theItem->loc.y)
               <= distance * distance) {
...
short reflectBolt(short targetX, short targetY, pos listOfCoordinates[], short kinkCell, boolean retracePath) {
  ...
  if (retracePath) {
    ...
    pos target = (pos){.x = targetX + (targetX - listOfCoordinates[kinkCell].x),
...
boolean updateBolt(bolt *theBolt, creature *caster, short x, short y, boolean boltInView, boolean alreadyReflected,
    ...
    case BE_ENTRANCEMENT:
      if (monst == &player) {
        ...
        monst->status[STATUS_ENTRANCED] = monst->maxStatus[STATUS_ENTRANCED]
            = staffEntrancementDuration(theBolt->magnitude * FP_FACTOR);
        wakeUp(monst);
        if (canSeeMonster(monst)) {
    ...
    case BE_DISCORD:
      if (!(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))) {
        monst->status[STATUS_DISCORDANT] = monst->maxStatus[STATUS_DISCORDANT]
            = max(staffDiscordDuration(theBolt->magnitude * FP_FACTOR), monst->status[STATUS_DISCORDANT]);
        if (canSeeMonster(monst)) {
...
void detonateBolt(bolt *theBolt, creature *caster, short x, short y, boolean *autoID) {
  ...
  const fixpt POW_OBSTRUCTION[]
      = {// 0.8^x, with x from 2 to 40:
         41943, 33554, 26843, 21474, 17179, 13743, 10995, 8796, 7036, 5629, 4503, 3602, 2882, 2305,
         1844,  1475,  1180,  944,   755,   604,   483,   386,  309,  247,  198,  158,  126,  101,
         81,    64,    51,    41,    33,    26,    21,    17,   13,   10,   8,    6,    5};
...
boolean zap(pos originLoc, pos targetLoc, bolt *theBolt, boolean hideDetails, boolean reverseBoltDir) {
  ...
  if (reverseBoltDir) {
    ...
    short numCellsTmp = getLineCoordinates(listOfCoordinatesTmp, targetLoc, originLoc,
                                           (hideDetails ? &boltCatalog[BOLT_NONE] : theBolt));
    numCells = -1;
  ...
  if (theBolt->boltEffect == BE_BLINKING) {
    if (cellHasTerrainFlag(listOfCoordinates[0].x, listOfCoordinates[0].y,
  ...
  if (boltColor) {
    for (i = 0; i < initialBoltLength; i++) {
      ...
      boltLightRadius = 50LL * ((3 * FP_FACTOR) + (theBolt->magnitude * FP_FACTOR) * 4 / 3) * (initialBoltLength - i)
                        / initialBoltLength / FP_FACTOR;
      boltLights[i].lightRadius.lowerBound = boltLights[i].lightRadius.upperBound = boltLightRadius;
      // boltLights[i].lightRadius.lowerBound = boltLights[i].lightRadius.upperBound = 50 * (3 + theBolt->magnitude
      // * 1.33) * (initialBoltLength - i) / initialBoltLength; printf("\nStandard: %i, attempted new: %lli",
      // boltLights[i].lightRadius.lowerBound, boltLightRadius);
    }
  }
  ...
  for (i = 0; i < numCells; i++) {
    ...
    if (monst && !(theBolt->flags & BF_NEVER_REFLECTS) && projectileReflects(shootingMonst, monst)
      ...
      if (boltInView) {
        monsterName(monstName, monst, true);
        sprintf(buf, "%s deflect%s the %s", monstName, (monst == &player ? "" : "s"),
    ...
    if (cellHasTerrainFlag(x, y, (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION)) && theBolt->boltEffect == BE_TUNNELING
        && tunnelize(x, y)) {
    ...
    if (i + 1 < numCells && !(theBolt->flags & BF_NEVER_REFLECTS)) {
      ...
      y2 = listOfCoordinates[i + 1].y;
      if (cellHasTerrainFlag(x2, y2, (T_OBSTRUCTS_VISION | T_OBSTRUCTS_PASSABILITY))
          && (projectileReflects(shootingMonst, NULL) || cellHasTMFlag(x2, y2, TM_REFLECTS_BOLTS)
      ...
      for (j = i; j < i + boltLength + 2; j++) { // j can imply a bolt tip position that is off the map
        ...
        for (k = min(j, boltLength + 2); k >= j - i; k--) {
          if (playerCanSee(listOfCoordinates[j - k].x, listOfCoordinates[j - k].y)) {
            if (boltColor) {
              hiliteCell(listOfCoordinates[j - k].x, listOfCoordinates[j - k].y, boltColor,
...
// first qualifying.
boolean nextTargetAfter(short *returnX, short *returnY, short targetX, short targetY, boolean targetEnemies,
                        boolean targetAllies, boolean targetItems, boolean targetTerrain, boolean requireOpenPath,
  ...
  for (int i = reverseDirection ? targetCount - 1 : 0; reverseDirection ? i >= 0 : i < targetCount;
    ...
    if ((newX != player.loc.x || newY != player.loc.y) && (newX != targetX || newY != targetY)
...
short hiliteTrajectory(const pos coordinateList[DCOLS], short numCells, boolean eraseHiliting, const bolt *theBolt,
                       const color *hiliteColor) {
  short x, y, i;
  creature *monst;
...
boolean moveCursor(boolean *targetConfirmed, boolean *canceled, boolean *tabKey, pos *targetLoc, rogueEvent *event,
                   buttonState *state, boolean colorsDance, boolean keysMoveCursor, boolean targetCanLeaveMap) {
  ...
  do {
    ...
    if (theEvent.eventType == MOUSE_UP || theEvent.eventType == MOUSE_ENTERED_CELL) {
      if (theEvent.param1 >= 0 && theEvent.param1 < mapToWindowX(0) && theEvent.param2 >= 0
          && theEvent.param2 < ROWS - 1 && isPosInMap(rogue.sidebarLocationList[theEvent.param2])) {
    ...
    if (sidebarHighlighted
        ...
        && (!cellHasTMFlag(rogue.cursorLoc.x, rogue.cursorLoc.y, TM_LIST_IN_SIDEBAR)
            || !playerCanSeeOrSense(rogue.cursorLoc.x, rogue.cursorLoc.y))) {
...
pos pullMouseClickDuringPlayback(void) {
  ...
  return (pos){.x = windowToMapX(theEvent.param1), .y = windowToMapY(theEvent.param2)};
}
...
static boolean creatureIsTargetable(creature *monst) {
  return monst != NULL && canSeeMonster(monst) && monst->depth == rogue.depthLevel
...
boolean chooseTarget(pos *returnLoc, short maxDistance, boolean stopAtTarget, boolean autoTarget, boolean targetAllies,
                     const bolt *theBolt, const color *trajectoryColor) {
  ...
  if (autoTarget) {
    if (creatureIsTargetable(rogue.lastTarget) && (targetAllies == (rogue.lastTarget->creatureState == MONSTER_ALLY))) {
      ...
      if (nextTargetAfter(&newX, &newY, targetLoc.x, targetLoc.y, !targetAllies, targetAllies, false, false, true,
                          false)) {
  ...
  do {
    ...
    if (monst != NULL && monst != &player && canSeeMonster(monst)) {
      focusedOnSomething = true;
    } else if (playerCanSeeOrSense(targetLoc.x, targetLoc.y) && (pmapAt(targetLoc)->flags & HAS_ITEM)
               || cellHasTMFlag(targetLoc.x, targetLoc.y, TM_LIST_IN_SIDEBAR)) {
      focusedOnSomething = true;
...
boolean hitMonsterWithProjectileWeapon(creature *thrower, creature *monst, item *theItem) {
  ...
  if (thrownWeaponHit) {
    damage = monst->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE)
                 ? 0
...
void throwItem(item *theItem, creature *thrower, pos targetLoc, short maxDistance) {
  ...
  // Using BOLT_NONE for throws because all flags are off, which means we'll try to avoid all obstacles in front of the
  // target
  pos listOfCoordinates[MAX_BOLT_LENGTH];
  ...
  if (thrower != &player && (pmapAt(originLoc)->flags & IN_FIELD_OF_VIEW)) {
  ...
  for (i = 0; i < numCells && i < maxDistance; i++) {
    ...
    if (pmap[x][y].flags & (HAS_MONSTER | HAS_PLAYER)) {
      ...
      if (!(monst->bookkeepingFlags & MB_SUBMERGED)) {
        ...
        //              if (projectileReflects(thrower, monst)) { // if it scores another reflection roll, reflect at
        //              caster
  ...
  if (maxDistance > 0) {
    ...
    for (i = 0; i < numCells; i++) {
      ...
      if (pmap[x][y].flags & DISCOVERED) {
        ...
        if ((tFlags & T_LAVA_INSTA_DEATH) && !(tFlags & (T_ENTANGLES | T_AUTO_DESCENT))
...
boolean useStaffOrWand(item *theItem, boolean *commandsRecorded) {
  ...
  pos originLoc = player.loc;
  pos zapTarget;
  confirmedTarget = chooseTarget(&zapTarget, maxDistance, false, autoTarget, targetAllies,
                                 (boltKnown ? &theBolt : &boltCatalog[BOLT_NONE]), &trajectoryHiliteColor);
...
void summonGuardian(item *theItem) {
  ...
  monst->status[STATUS_LIFESPAN_REMAINING] = monst->maxStatus[STATUS_LIFESPAN_REMAINING]
      = charmGuardianLifespan(netEnchant(theItem));
  pmapAt(monst->loc)->flags |= HAS_MONSTER;
  fadeInMonster(monst);
}
...
void useCharm(item *theItem) {
  ...
  case CHARM_NEGATION:
    negationBlast("your charm",
                  charmNegationRadius(enchant) + 1); // Add 1 because otherwise radius 1 would affect only the player.
    break;
  default:
    break;
  }
...
void apply(item *theItem, boolean recordCommands) {
  ...
  case FOOD:
    ...
    player.status[STATUS_NUTRITION]
        = min(foodTable[theItem->kind].power + player.status[STATUS_NUTRITION], STOMACH_SIZE);
...
void readScroll(item *theItem) {
  ...
  case SCROLL_ENCHANTING:
    ...
    case CHARM:
      theItem->enchant1 += enchantMagnitude();
      theItem->charges
          = min(0, theItem->charges); // Enchanting instantly recharges charms.
  ...
  case SCROLL_SUMMON_MONSTER:
    for (j = 0; j < 25 && numberOfMonsters < 3; j++) {
      for (i = 0; i < 8; i++) {
        ...
        if (!cellHasTerrainFlag(x, y, T_OBSTRUCTS_PASSABILITY) && !(pmap[x][y].flags & HAS_MONSTER) && rand_percent(10)
            && (numberOfMonsters < 3)) {
          monst = spawnHorde(
...
item *promptForItemOfType(unsigned short category, unsigned long requiredFlags, unsigned long forbiddenFlags,
                          char *prompt, boolean allowInventoryActions) {
  char keystroke;
  item *theItem;
...
item *itemAtLoc(short x, short y) {
  ...
  for (theItem = floorItems->nextItem; theItem != NULL && (theItem->loc.x != x || theItem->loc.y != y);
...
boolean equipItem(item *theItem, boolean force, item *unequipHint) {
  ...
  } else if (theItem->category & RING && unequipHint && rogue.ringLeft && rogue.ringRight
...
void deleteItem(item *theItem) { free(theItem); }
