...
void applyInstantTileEffectsToCreature(creature *monst) {
  ...
  // Water puts out fire.
  if (cellHasTMFlag(*x, *y, TM_EXTINGUISHES_FIRE) && monst->status[STATUS_BURNING] && !monst->status[STATUS_LEVITATING]
      && !(monst->info.flags & MONST_ATTACKABLE_THRU_WALLS) && !(monst->info.flags & MONST_FIERY)) {
  ...
  if (monst == &player && cellHasTerrainFlag(*x, *y, T_RESPIRATION_IMMUNITIES) && rogue.armor
    ...
    // paralysis gas
    if (cellHasTerrainFlag(*x, *y, T_CAUSES_PARALYSIS) && !(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))
        && !(monst->bookkeepingFlags & MB_SUBMERGED)) {
...
void monsterEntersLevel(creature *monst, short n) {
  ...
  if (!pit && (pmapAt(monst->loc)->flags & (HAS_PLAYER | HAS_MONSTER))
    ...
    // DEBUG printf("\nBumped a creature (%s) from (%i, %i) to (%i, %i).", prevMonst->info.monsterName, monst->loc.x,
    // monst->loc.y, prevMonst->loc.x, prevMonst->loc.y);
  }
...
void playerTurnEnded() {
  ...
  do {
    ...
    for (creatureIterator it = iterateCreatures(monsters); hasNextCreature(it);) {
      creature *monst = nextCreature(&it);
      if ((monst->bookkeepingFlags & MB_BOUND_TO_LEADER) && (!monst->leader || !(monst->bookkeepingFlags & MB_FOLLOWER))
      ...
      if (canSeeMonster(monst) && !(monst->bookkeepingFlags & (MB_WAS_VISIBLE | MB_ALREADY_SEEN))) {
        if (monst->creatureState != MONSTER_ALLY) {
          ...
          if (rogue.cautiousMode || rogue.automationActive) {
            ...
            sprintf(buf, "you %s a%s %s", playerCanDirectlySee(monst->loc.x, monst->loc.y) ? "see" : "sense",
