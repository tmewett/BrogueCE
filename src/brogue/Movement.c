      ...
      if (pmap[x][y].rememberedItemCategory) {
        if (player.status[STATUS_HALLUCINATING] && !rogue.playbackOmniscience) {
          ...
          describedItemBasedOnParameters(pmap[x][y].rememberedItemCategory, pmap[x][y].rememberedItemKind,
                                         pmap[x][y].rememberedItemQuantity, pmap[x][y].rememberedItemOriginDepth,
                                         object);
        }
      } else {
    ...
    if (player.status[STATUS_LEVITATING] <= 1 && player.status[STATUS_IMMUNE_TO_FIRE] <= 1) {
      ...
      for (i = 0; i < 8; i++) {
        ...
        if (coordinatesAreInMap(newestX, newestY) && (pmap[newestX][newestY].flags & (DISCOVERED | MAGIC_MAPPED))
            ...
            && !((pmap[newestX][newestY].flags & HAS_MONSTER) && canSeeMonster(monsterAtLoc((pos){newestX, newestY}))
  ...
  if (((!cellHasTerrainFlag(newX, newY, T_OBSTRUCTS_PASSABILITY)
    ...
    if (pmap[newX][newY].flags & (DISCOVERED | MAGIC_MAPPED) && player.status[STATUS_LEVITATING] <= 1
      ...
      return false;
               ...
               && (!rogue.armor || !(rogue.armor->flags & ITEM_RUNIC) || !(rogue.armor->flags & ITEM_RUNIC_IDENTIFIED)
    ...
    if (rogue.downLoc.x == newX && rogue.downLoc.y == newY) {
      ...
      if (defender && defender->creatureState == MONSTER_ALLY) { // Swap places with ally.
        ...
        // getQualifyingLocNear(loc, player.loc.x, player.loc.y, true, NULL, forbiddenFlagsForMonster(&(defender->info))
        // & ~(T_IS_DF_TRAP | T_IS_DEEP_WATER | T_SPONTANEOUSLY_IGNITES), HAS_MONSTER, false, false); defender->loc.x =
        // loc[0]; defender->loc.y = loc[1];
        pmapAt(defender->loc)->flags |= HAS_MONSTER;
      }
    ...
    for (j = 0; j < DROWS; j++) {
      ...
      if ((tFlags & T_LAVA_INSTA_DEATH)
          ...
          && max(monst->status[STATUS_LEVITATING], monst->status[STATUS_IMMUNE_TO_FIRE])
                 < (rogue.mapToShore[i][j] + distanceBetween((pos){i, j}, monst->loc) * monst->movementSpeed / 100)) {
...
void getFOVMask(char grid[DCOLS][DROWS], short xLoc, short yLoc, fixpt maxRadius, unsigned long forbiddenTerrain,
  ...
  for (int i = 1; i <= 8; i++) {
    scanOctantFOV(grid, loc.x, loc.y, i, maxRadius, 1, LOS_SLOPE_GRANULARITY * -1, 0, forbiddenTerrain, forbiddenFlags,
                  cautiousOnWalls);
  }
}
...
void scanOctantFOV(char grid[DCOLS][DROWS], short xLoc, short yLoc, short octant, fixpt maxRadius,
  ...
  for (i = iStart; i <= iEnd; i++) {
    ...
    if (!cellObstructed && !currentlyLit) { // next column slope starts here
      newStartSlope
          = (long int)((LOS_SLOPE_GRANULARITY * (i)-LOS_SLOPE_GRANULARITY / 2) / (columnsRightFromOrigin * 2 + 1) * 2);
      currentlyLit = true;
