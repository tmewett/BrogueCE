        ...
        if (passMap[i][j] && (pmap[i][j].flags & IS_CHOKEPOINT)) {
          for (dir = 0; dir < 4; dir++) {
            ...
            if (coordinatesAreInMap(newX, newY) && passMap[newX][newY] && !(pmap[newX][newY].flags & IS_CHOKEPOINT)) {
              ...
              // Pretend (i, j) is blocked by changing passMap, and run a flood-fill cell count starting on (newX,
              // newY). Keep track of the flooded region in grid[][].
              zeroOutGrid(grid);
              passMap[i][j] = false;
              ...
              if (cellCount >= 4) {
                // Now, on the chokemap, all of those flooded cells should take the lesser of their current value or
    ...
    if (!grid[x][y]) {
      for (d = 0; d <= 1; d++) { // Try a horizontal door, and then a vertical door.
        ...
        if (coordinatesAreInMap(newX, newY) && coordinatesAreInMap(oppX, oppY) && grid[newX][newY] == 1
          ...
          if (pathMap[oppX][oppY] > minimumPathingDistance) { // and if the pathing distance between the two flanking
                                                              // floor tiles exceeds minimumPathingDistance,
  ...
  if (featureFlags & MF_BUILD_IN_WALLS) { // If we're supposed to build in a wall...
    if (!interior[x][y] && (pmap[x][y].machineNumber == 0 || pmap[x][y].machineNumber == machineNumber)
      ...
      for (dir = 0; dir < 4; dir++) {
        newX = x + nbDirs[dir][0];
        newY = y + nbDirs[dir][1];
        if (coordinatesAreInMap(
                newX, newY) // ...and it's next to an interior spot or permitted elsewhere and next to passable spot...
...
boolean fillInteriorForVestibuleMachine(char interior[DCOLS][DROWS], short bp, short originX, short originY) {
  ...
  totalFreq
      = rand_range(blueprintCatalog[bp].roomSize[0], blueprintCatalog[bp].roomSize[1]); // Keeps track of the goal size.
...
typedef struct machineData {
  ...
  char occupied[DCOLS][DROWS]; // This keeps track of what is within the personal space of a previously built feature in
                               // the same machine.
...
boolean buildAMachine(enum machineTypes bp, short originX, short originY, unsigned long requiredMachineFlags,
    ...
    if (--failsafe <= 0) {
      ...
      if (D_MESSAGE_MACHINE_GENERATION) {
        if (chooseBP || chooseLocation) {
          printf("\nDepth %i: Failed to build a machine; gave up after 10 unsuccessful attempts to find a suitable "
                 "blueprint and/or location.",
                 rogue.depthLevel);
        } else {
    ...
    if (blueprintCatalog[bp].flags & BP_ROOM) {
      ...
      if (chooseLocation) {
        ...
        for (i = 0; i < DCOLS; i++) {
          for (j = 0; j < DROWS && totalFreq < 50; j++) {
            if ((pmap[i][j].flags & IS_GATE_SITE) && !(pmap[i][j].flags & IS_IN_MACHINE)
              ...
              // DEBUG printf("\nDepth %i: Gate site qualified with interior size of %i.", rogue.depthLevel,
              // chokeMap[i][j]);
        ...
        if (totalFreq) {
          ...
          if (D_MESSAGE_MACHINE_GENERATION)
            printf("\nDepth %i: Failed to build a machine; there was no eligible door candidate for the chosen room "
                   "machine from blueprint %i.",
                   rogue.depthLevel, bp);
          free(p);
          return false;
        }
      ...
      if (chooseLocation) {
        ...
        if (D_MESSAGE_MACHINE_GENERATION)
          printf("\nDepth %i: ERROR: Attempted to build a door machine from blueprint %i without a location being "
                 "provided.",
                 rogue.depthLevel, bp);
        free(p);
        return false;
      ...
      do {
        ...
        // If we're not choosing the blueprint, then don't bother with the locationFailsafe; just use the higher-level
        // failsafe.
  ...
  // This is the point of no return. Back up the level so it can be restored if we have to abort this machine after this
  // point.
  ...
  // accordingly) and figure out the 33rd and 67th percentiles for features that want to be near or far from the origin.
  if (!distanceMap) {
    distanceMap = allocGrid();
  }
  fillGrid(distanceMap, 0);
  ...
  for (feat = 0; feat < blueprintCatalog[bp].featureCount; feat++) {
    ...
    do { // If the MF_REPEAT_UNTIL_NO_PROGRESS flag is set, repeat until we fail to build the required number of
      ...
      for (i = 0; i < DCOLS; i++) {
        for (j = 0; j < DROWS; j++) {
          if (cellIsFeatureCandidate(i, j, originX, originY, distanceBound, p->interior, p->occupied, p->viewMap,
                                     distanceMap, machineNumber, feature->flags, blueprintCatalog[bp].flags)) {
            qualifyingTileCount++;
      ...
      if (feature->flags & MF_EVERYWHERE & ~MF_BUILD_AT_ORIGIN) {
        // Generate everywhere that qualifies -- instead of randomly picking tiles, keep spawning until we run out of
        // eligible tiles.
      ...
      for (instance = 0; (generateEverywhere || instance < instanceCount) && qualifyingTileCount > 0;) {
        ...
        // OK, if placement was successful, clear some personal space around the feature so subsequent features can't be
        // generated too close. Personal space of 0 means nothing gets cleared, 1 means that only the tile itself gets
        // cleared, and 2 means the 3x3 grid centered on it.
              ...
              if (coordinatesAreInMap(i, j)) {
                if (p->candidates[i][j]) {
                  brogueAssert(!p->occupied[i][j]
                               || (i == originX
                                   && j == originY)); // Candidates[][] should never be true where occupied[][] is true.
                  p->candidates[i][j] = false;
                  qualifyingTileCount--;
                }
                p->occupied[i][j] = true;
        ...
        if (DFSucceeded && terrainSucceeded) { // Proceed only if the terrain stuff for this instance succeeded.
          ...
          if ((feature->flags & MF_GENERATE_ITEM)
            ...
            if (adoptiveItem && (feature->flags & MF_ADOPT_ITEM) && (blueprintCatalog[bp].flags & BP_ADOPT_ITEM)) {
              ...
              p->spawnedItems[itemCount]
                  = theItem; // Keep a list of generated items so that we can delete them all if construction fails.
              itemCount++;
            }
          ...
          if (feature->flags & (MF_OUTSOURCE_ITEM_TO_MACHINE | MF_BUILD_VESTIBULE)) {
            ...
            for (i = 10; i > 0; i--) {
              ...
              if ((feature->flags & MF_OUTSOURCE_ITEM_TO_MACHINE) && theItem) {
                ...
                success
                    = buildAMachine(-1, featX, featY, BP_VESTIBULE, NULL, p->spawnedItemsSub, p->spawnedMonstersSub);
              }
            ...
            if (!i) {
              if (D_MESSAGE_MACHINE_GENERATION)
                printf("\nDepth %i: Failed to place blueprint %i because it requires an adoptive machine and we "
                       "couldn't place one.",
                       rogue.depthLevel, bp);
              // failure! abort!
  ...
  freeGrid(distanceMap);
  if (D_MESSAGE_MACHINE_GENERATION)
    printf("\nDepth %i: Built a machine from blueprint %i with an origin at (%i, %i).", rogue.depthLevel, bp, originX,
           originY);
      ...
      if (!cellHasTerrainFlag(i, j, (T_CAN_BE_BRIDGED | T_PATHING_BLOCKER)) && !pmap[i][j].machineNumber) {
        ...
        for (k = i + 1;
             ...
             && cellHasTerrainFlag(
                 k, j - 1,
                 (T_CAN_BE_BRIDGED
                  | T_OBSTRUCTS_PASSABILITY)) // Only chasms or walls are permitted next to the length of the bridge.
        ...
        if (k < DROWS && (k - j > 3) && foundExposure && !cellHasTerrainFlag(i, k, T_PATHING_BLOCKER | T_CAN_BE_BRIDGED)
...
void digDungeon() {
  ...
  // propose a position, and then check via a flood fill that the level would remain connected with that placement (i.e.
  // that each passable tile can still be reached). If not, make 9 more placement attempts before abandoning that lake
  // and proceeding to generate the next smaller one.
  // Canvas sizes start at 30x15 and decrease by 2x1 at a time down to a minimum of 20x10. Min generated size is always
  // 4x4.
  ...
  if (feat->tile) {
    if (feat->layer == GAS) {
      ...
      if (!blocking || !levelIsDisconnectedWithBlockingMap(blockingMap, false)) {
        ...
        // succeeded = fillSpawnMap(feat->layer, feat->tile, blockingMap, (feat->flags & DFF_BLOCKED_BY_OTHER_LAYERS),
        // refreshCell, (feat->flags & DFF_SUPERPRIORITY));
        ...
        succeeded
            = true; // fail ONLY if we blocked the level. We succeed even if, thanks to priority, nothing gets built.
      } else {
        succeeded = false;
      }
    }
...
void restoreMonster(creature *monst, short **mapToStairs, short **mapToPit) {
  ...
  if (monst->status[STATUS_ENTERS_LEVEL_IN] > 0) {
    ...
    if (theMap) {
      // STATUS_ENTERS_LEVEL_IN accounts for monster speed; convert back to map distance and subtract from distance to
      // stairs
      turnCount
          = (theMap[monst->loc.x][monst->loc.y] - (monst->status[STATUS_ENTERS_LEVEL_IN] * 100 / monst->movementSpeed));
