      ...
      if (monsterGrid[i][j]) {
        for (int dir = 0; dir < 4; dir++) {
          ...
          if (coordinatesAreInMap(newX, newY) && !eligibleGrid[newX][newY] && !monsterGrid[newX][newY]
              && !(pmap[newX][newY].flags & (HAS_PLAYER | HAS_MONSTER)) && !monsterAvoids(monst, (pos){newX, newY})) {
  ...
  if (sneakAttack || defenderWasAsleep || defenderWasParalyzed || lungeAttack || attackHit(attacker, defender)) {
    ...
    if (inflictDamage(attacker, defender, damage, &red, false)) { // if the attack killed the defender
      ...
      if (!rogue.blockCombatText && (canSeeMonster(attacker) || canSeeMonster(defender))) {
        attackVerb(
            verb, attacker,
            max(damage - (attacker->info.damage.lowerBound * monsterDamageAdjustmentAmount(attacker) / FP_FACTOR), 0)
                * 100
...
boolean inflictDamage(creature *attacker, creature *defender, short damage, const color *flashColor,
  ...
  if (defender->currentHP <= damage) { // killed
    ...
    if (defender != &player && defender->creatureState != MONSTER_ALLY && defender->info.flags & MONST_FLEES_NEAR_DEATH
...
// Use "administrativeDeath" if the monster is being deleted for administrative purposes, as opposed to dying as a
// result of physical actions. AdministrativeDeath means the monster simply disappears, with no messages, dropped item,
// DFs or other effect.
