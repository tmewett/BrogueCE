...
void formatRecentMessages(char buffer[][COLS*2], size_t height, short *linesFormatted, short *latestMessageLines) {
    short lines, bufferCursor, messagesFormatted;
    char folded[COLS*2], wrapped[COLS*2];
    archivedMessage *nextMessage;

    bufferCursor = height - 1;
    messagesFormatted = 0;

    while (bufferCursor >= 0 && messagesFormatted < MESSAGE_ARCHIVE_ENTRIES) {
        nextMessage = getArchivedMessage(messagesFormatted + 1);
        if (!nextMessage->message[0]) {
            break;
        }

        formatCountedMessage(folded, COLS*2, nextMessage);

        lines = wrapText(wrapped, folded, DCOLS);
        splitLines(lines, wrapped, buffer, bufferCursor);

        if (bufferCursor == height - 1 && latestMessageLines) {
