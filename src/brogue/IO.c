...
short actionMenu(short x, boolean playingBack) {
  ...
  do {
    ...
    if (KEYBOARD_LABELS) {
      sprintf(buttons[buttonCount].text, "  %s\\: %s[%s] Hide color effects  ", yellowColorEscape, whiteColorEscape,
              rogue.trueColorMode ? "X" : " ");
    } else {
    ...
    buttonCount++;
    if (KEYBOARD_LABELS) {
      sprintf(buttons[buttonCount].text, "  %s]: %s[%s] Display stealth range  ", yellowColorEscape, whiteColorEscape,
    ...
    if (hasGraphics) {
      if (KEYBOARD_LABELS) {
        sprintf(buttons[buttonCount].text, "  %sG: %s[%c] Enable graphics  ", yellowColorEscape, whiteColorEscape,
                " X~"[graphicsMode]);
      } else {
    ...
    do {
      ...
      if (!playingBack) {
        ...
        if (isPosInMap(rogue.cursorLoc)) {
          ...
          //                  steps = getPlayerPathOnMap(path, playerPathingMap, pathDestination[0], pathDestination[1])
          //                  - 1; // Get new path. reversePath(path, steps);   // Flip it around, back-to-front.
      ...
      // Get the input!
      rogue.playbackMode = playingBack;
      doEvent = moveCursor(&targetConfirmed, &canceled, &tabKey, &rogue.cursorLoc, &theEvent, &state, !textDisplayed,
                           rogue.cursorMode, true);
      rogue.playbackMode = false;
...
void executeKeystroke(signed long keystroke, boolean controlKey, boolean shiftKey) {
  ...
  case TRUE_COLORS_KEY:
    ...
    if (rogue.trueColorMode) {
      ...
      messageWithColor(
          KEYBOARD_LABELS ? "Color effects enabled. Press '\\' again to disable." : "Color effects enabled.", &teal, 0);
    }
    break;
  case STEALTH_RANGE_KEY:
    ...
    if (rogue.displayStealthRangeMode) {
      messageWithColor(
          KEYBOARD_LABELS ? "Stealth range displayed. Press ']' again to hide." : "Stealth range displayed.", &teal, 0);
    } else {
  ...
  case SEED_KEY:
    ...
    // parseFile();
    // DEBUG spawnDungeonFeature(player.loc.x, player.loc.y, &dungeonFeatureCatalog[DF_METHANE_GAS_ARMAGEDDON], true,
    // false);
    printSeed();
    break;
  case EASY_MODE_KEY:
...
void updateMessageDisplay() {
  ...
  for (i = 0; i < MESSAGE_LINES; i++) {
    ...
    for (j = m = 0; displayedMessage[i][m] && j < DCOLS; j++, m++) {
      ...
      plotCharWithColor(displayedMessage[i][m], (windowpos){mapToWindowX(j), MESSAGE_LINES - i - 1}, &messageColor,
                        &black);
    }
    for (; j < DCOLS; j++) {
    ...
    do {
      ...
      for (theItem = floorItems->nextItem; theItem != NULL; theItem = theItem->nextItem) {
        if ((playerCanDirectlySee(theItem->loc.x, theItem->loc.y)
             || (indirectVision && (playerCanSeeOrSense(theItem->loc.x, theItem->loc.y) || rogue.playbackOmniscience)))
