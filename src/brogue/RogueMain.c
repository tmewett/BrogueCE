...
void startLevel(short oldLevelNumber, short stairDirection) {
  ...
  if (oldLevelNumber != rogue.depthLevel) {
    ...
    for (flying = 0; flying <= 1; flying++) {
      fillGrid(mapToStairs, 0);
      calculateDistances(mapToStairs, px, py, (flying ? T_OBSTRUCTS_PASSABILITY : T_PATHING_BLOCKER) | T_SACRED, NULL,
                         true, true);
  ...
  if (stairDirection == 0) { // fell into the level
    ...
    if (!placedPlayer) {
      getQualifyingPathLocNear(&loc.x, &loc.y, player.loc.x, player.loc.y, true, T_DIVIDES_LEVEL, 0, T_PATHING_BLOCKER,
                               (HAS_MONSTER | HAS_ITEM | HAS_STAIRS | IS_IN_MACHINE), false);
    }
  }
  player.loc = loc;
  ...
  while (next != NULL) {
    ...
    if (decedent->bookkeepingFlags & MB_HAS_DIED) {
      ...
      if (decedent->leader == &player && !(decedent->info.flags & MONST_INANIMATE)
          && (decedent->bookkeepingFlags & MB_HAS_SOUL) && !(decedent->bookkeepingFlags & MB_ADMINISTRATIVE_DEATH)) {
