...
void monsterDetails(char buf[], creature *monst) {
    ...
    if (monsterIsNegatable(monst)) {
        ...
        if (pack.hasNegationWand && !((monst->info.flags & MONST_REFLECT_4) && (monst->info.flags & MONST_ALWAYS_USE_ABILITY))) {
    ...
    if ((monst->info.flags & MONST_REFLECT_4) && (monst->info.flags & MONST_ALWAYS_USE_ABILITY)
        && (pack.StaffCount || pack.wandCount)) {
