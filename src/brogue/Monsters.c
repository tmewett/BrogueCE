      ...
      if (monsterAvoids(monst, (pos){x, newY}) || (!willingToAttackPlayer && pmap[x][newY].flags & HAS_PLAYER)
        ...
        if (monsterAvoids(monst, (pos){newX, y}) || (!willingToAttackPlayer && pmap[newX][y].flags & HAS_PLAYER)
          ...
          if (monsterAvoids(monst, (pos){x - 1, newY})
              || (!willingToAttackPlayer && pmap[x - 1][newY].flags & HAS_PLAYER) || !moveMonster(monst, -1, dy)) {
            if (monsterAvoids(monst, (pos){x + 1, newY})
                || (!willingToAttackPlayer && pmap[x + 1][newY].flags & HAS_PLAYER) || !moveMonster(monst, 1, dy)) {
              return false;
            }
      ...
      if (monsterAvoids(monst, (pos){newX, y}) || (!willingToAttackPlayer && pmap[newX][y].flags & HAS_PLAYER)
        ...
        if (monsterAvoids(monst, (pos){x, newY}) || (!willingToAttackPlayer && pmap[x][newY].flags & HAS_PLAYER)
          ...
          if (monsterAvoids(monst, (pos){newX, y - 1})
              || (!willingToAttackPlayer && pmap[newX][y - 1].flags & HAS_PLAYER) || !moveMonster(monst, dx, -1)) {
            if (monsterAvoids(monst, (pos){newX, y + 1})
                || (!willingToAttackPlayer && pmap[newX][y + 1].flags & HAS_PLAYER) || !moveMonster(monst, dx, 1)) {
              return false;
            }
...
void updateMonsterState(creature *monst) {
  ...
  if ((monst->creatureState == MONSTER_WANDERING) && awareOfPlayer && (pmapAt(player.loc)->flags & IN_FIELD_OF_VIEW)) {
    ...
    if (monst->leader == &player) {
      monst->creatureState
          = MONSTER_ALLY; // Reset state if a discorded ally steals an item and then loses it (probably in deep water)
    } else {
      alertMonster(monst);
    }
    ...
    if (ally != monst && ally != enemy && monstersAreTeammates(monst, ally) && monstersAreEnemies(ally, enemy)
      ...
      if (!alternateDirectionExists) {
        ...
        for (creatureIterator it2 = iterateCreatures(monsters); !handledPlayer || hasNextCreature(it2);) {
          ...
          handledPlayer = true;
          if (ally != otherEnemy && monst != otherEnemy && enemy != otherEnemy && monstersAreEnemies(ally, otherEnemy)
...
void moveAlly(creature *monst) {
  ...
  if (cellHasTerrainFlag(x, y,
                         (T_HARMFUL_TERRAIN & ~(T_IS_FIRE | T_CAUSES_DAMAGE | T_CAUSES_PARALYSIS | T_CAUSES_CONFUSION)))
  ...
  if (shortestDistance == 1) {
    if (closestMonster->movementSpeed < monst->movementSpeed
      ...
      leashLength++; // If the ally is adjacent to a monster at the end of its leash, it shouldn't be prevented from
                     // attacking.
    }
  }
  ...
  if (((defender && (defender->info.flags & MONST_ATTACKABLE_THRU_WALLS))
    ...
    if (defender) {
      if (canPass(monst, defender)) {
        ...
        if (monsterAvoids(defender,
          ...
          getQualifyingPathLocNear(
              &(defender->loc.x), &(defender->loc.y), x, y, true, forbiddenFlagsForMonster(&(defender->info)),
              HAS_PLAYER, forbiddenFlagsForMonster(&(defender->info)), (HAS_PLAYER | HAS_MONSTER | HAS_STAIRS), false);
        } else {
          defender->loc.x = x;
          defender->loc.y = y;
        }
      ...
      if (swarmDirection != NO_DIRECTION) {
        ...
        // Attack!
        for (i = 0; i < 16; i++) {
          if (hitList[i] && monsterWillAttackTarget(monst, hitList[i]) && !(hitList[i]->bookkeepingFlags & MB_IS_DYING)
              && !rogue.gameHasEnded) {
...
void toggleMonsterDormancy(creature *monst) {
  ...
  if (removeCreature(dormantMonsters, monst)) {
    ...
    if (pmapAt(monst->loc)->flags & (HAS_MONSTER | HAS_PLAYER)) { // Occupied!
      ...
      // getQualifyingLocNear(loc, monst->loc.x, monst->loc.y, true, 0, T_PATHING_BLOCKER, (HAS_PLAYER | HAS_MONSTER),
      // false, false); monst->loc.x = loc[0]; monst->loc.y = loc[1];
    }
    ...
    case BE_DAMAGE:
      if ((boltCatalog[boltForItem(theItem)].flags & BF_FIERY) && (monst->status[STATUS_IMMUNE_TO_FIRE])
        ...
        if (staffDamageLow(enchant) >= monst->currentHP) {
          sprintf(newText, "\n     Your %s (%c) will %s %s in one hit.", theItemName, theItem->inventoryLetter,
                  (monst->info.flags & MONST_INANIMATE) ? "destroy" : "kill", monstName);
        } else {
          sprintf(newText, "\n     Your %s (%c) will hit %s for between %i%% and %i%% of $HISHER current health.",
                  theItemName, theItem->inventoryLetter, monstName, 100 * staffDamageLow(enchant) / monst->currentHP,
...
void monsterDetails(char buf[], creature *monst) {
  ...
  if ((monst->info.flags & MONST_RESTRICTED_TO_LIQUID)
    ...
    if (monst->info.abilityFlags & MA_POISONS) {
      ...
      if (i == 0) {
        ...
        sprintf(newText,
                "%s has a %i%% chance to poison you, typically poisons for %i turns, and at worst, could fatally "
                "poison you in %i hit%s.\n     ",
      ...
      combatMath
          = ((player.currentHP + (monst->info.damage.upperBound * monsterDamageAdjustmentAmount(monst) / FP_FACTOR) - 1)
             * FP_FACTOR)
      ...
      sprintf(newText,
              "%s has a %i%% chance to hit you, typically hits for %i%% of your current health, and at worst, could "
              "defeat you in %i hit%s.\n     ",
              capMonstName, combatMath2,
  ...
  if (playerKnownMaxDamage == 0) {
    ...
    sprintf(newText,
            "You have a %i%% chance to hit %s, typically hit for %i%% of $HISHER current health, and at best, could "
            "defeat $HIMHER in %i hit%s.",
