...
static void printSeedCatalogCsvLine(uint64_t seed, short depth, short quantity, char categoryName[50],
                                    char kindName[50], char enchantment[50], char runicName[50], char vaultNumber[10],
                                    char opensVaultNumber[10], char carriedByMonsterName[50], char allyStatusName[20],
                                    char mutationName[100]) {

  printf("%s,%llu,%i,%i,%s,%s,%s,%s,%s,%s,%s,%s,%s\n", gameConst->dungeonVersionString, (unsigned long long)seed, depth,
         quantity, categoryName, kindName, enchantment, runicName, vaultNumber, opensVaultNumber, carriedByMonsterName,
...
static void printSeedCatalogItem(item *theItem, creature *theMonster, boolean isCsvFormat) {
  ...
  if (pmapAt(theItem->loc)->machineNumber > 0) {
    ...
    if (pmapAt(theItem->loc)->layers[0] != ALTAR_SWITCH && pmapAt(theItem->loc)->layers[0] != ALTAR_SWITCH_RETRACTING
        && pmapAt(theItem->loc)->layers[0] != ALTAR_CAGE_RETRACTABLE && pmapAt(theItem->loc)->layers[0] != ALTAR_INERT
...
static void printSeedCatalogAltars(boolean isCsvFormat) {
  ...
  for (j = 0; j < DROWS; j++) {
    for (i = 0; i < DCOLS; i++) {
      if (pmap[i][j].layers[0] == RESURRECTION_ALTAR) {
        ...
        if (isCsvFormat) {
          printSeedCatalogCsvLine(rogue.seed, rogue.depthLevel, 1, "altar", "resurrection altar", NO_ENCHANTMENT_STRING,
                                  NO_RUNIC_STRING, vaultNumber, NO_OPENS_VAULT_STRING, NO_CARRIED_BY_MONSTER_STRING,
                                  NO_ALLY_STATUS_STRING, NO_MUTATION_STRING);
        } else {
  ...
  for (i = 0; i < 50; i++) {
    if (c_altars[i]) {
      ...
      if (isCsvFormat) {
        printSeedCatalogCsvLine(rogue.seed, rogue.depthLevel, 1, "altar", "commutation altar", NO_ENCHANTMENT_STRING,
                                NO_RUNIC_STRING, vaultNumber, NO_OPENS_VAULT_STRING, NO_CARRIED_BY_MONSTER_STRING,
                                NO_ALLY_STATUS_STRING, NO_MUTATION_STRING);
      } else {
...
void printSeedCatalog(uint64_t startingSeed, uint64_t numberOfSeedsToScan, unsigned int scanThroughDepth,
  ...
  sprintf(message,
          ...
          (unsigned long long)startingSeed, (unsigned long long)startingSeed + numberOfSeedsToScan - 1,
          scanThroughDepth, gameConst->versionString, gameConst->dungeonVersionString);
