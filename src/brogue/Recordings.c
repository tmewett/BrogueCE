...
static void seek(unsigned long seekTarget, enum recordingSeekModes seekMode) {
  ...
  while (!arrivedAtDestination && !rogue.gameHasEnded && !rogue.playbackOOS) {
    if (useProgressBar && !(rogue.playerTurnNumber % progressBarRefreshInterval)) {
      ...
      printProgressBar((COLS - 20) / 2, ROWS / 2, "[     Loading...   ]", rogue.playerTurnNumber - startTurnNumber,
  ...
  if (recordingInput->eventType == KEYSTROKE) {
    ...
    case STEALTH_RANGE_KEY:
      ...
      if (rogue.displayStealthRangeMode) {
        ...
        messageWithColor(
            KEYBOARD_LABELS ? "Stealth range hidden. Press ']' again to display." : "Stealth range hidden.", &teal, 0);
      }
      return true;
    case GRAPHICS_KEY:
      if (hasGraphics) {
    ...
    while (recordingLocation < lengthOfPlaybackFile && rogue.playerTurnNumber < rogue.howManyTurns
      ...
      if (recordingLocation / progressBarInterval != previousRecordingLocation / progressBarInterval
        ...
        printProgressBar((COLS - 20) / 2, ROWS / 2, "[     Loading...   ]", recordingLocation, lengthOfPlaybackFile,
                         &darkPurple, false);
        while (pauseBrogue(
            0)) { // pauseBrogue(0) is necessary to flush the display to the window in SDL, as well as look for inputs
          rogue.creaturesWillFlashThisTurn = false; // prevent monster flashes from showing up on screen
...
void describeKeystroke(unsigned char key, char *description) {
  ...
  const char descList[51][30]
      = {"up", "down", "left", "right", "up arrow", "left arrow", "down arrow", "right arrow", "upleft", "upright",
         ...
         "delete", "tab", "period", "open file", "numpad 0", "numpad 1", "numpad 2", "numpad 3", "numpad 4", "numpad 5",
