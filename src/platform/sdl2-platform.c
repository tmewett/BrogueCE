    ...
    while (SDL_PollEvent(&event)) {
        if (event.type == SDL_QUIT) {
            ...
            */
            ...
            int xcell = event.motion.x * COLS / windowWidth, ycell = event.motion.y * ROWS / windowHeight;
...
static boolean _pauseForMilliseconds(short ms) {
    ...
    if (lastEvent.eventType != EVENT_ERROR && lastEvent.eventType != MOUSE_ENTERED_CELL) {
